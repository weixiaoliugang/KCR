...
{ 
  ...
  SPI_InitTypeDef  SPI_InitStructure;
	...
	RCC_APB2PeriphClockCmd(RCC_APB2Periph_GPIOB|RCC_APB2Periph_GPIOG, ENABLE);	 //使能PB,D,G端口时钟
  	
			
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_12;				 //PB12上拉 防止W25X的干扰(很重要，位置一定就在此）
...
u8 NRF24L01_RxPacket(u8 *rxbuf)
{
	...
	SPI2_SetSpeed(SPI_BaudRatePrescaler_8); //spi速度为9Mhz（24L01的最大SPI时钟为10Mhz）   
	sta=NRF24L01_Read_Reg(STATUS);  //读取状态寄存器的值 
	NRF24L01_Write_Reg(WRITE_REG_NRF+STATUS,(sta&0xf1));//清除TX_DS或MAX_RT中断标志(很重要，目的：有0x00变为0x0e)
	if(sta&RX_OK)//接收到数据
	{		
		NRF24L01_Read_Buf(RD_RX_PLOAD,rxbuf,RX_PLOAD_WIDTH);//读取数
		NRF24L01_Write_Reg(FLUSH_RX,0xff);//清除RX FIFO寄存器 
		sta=NRF24L01_Read_Reg(STATUS);  //读取状态寄存器的值 
	  NRF24L01_Write_Reg(WRITE_REG_NRF+STATUS,(sta&0xf1));//清除TX_DS或MAX_RT中断标
		return 0;
...
void NRF24L01_RX_Mode(void)
{
	  ...
	  NRF24L01_CE_Low;
    //NRF24L01_Write_Reg(WRITE_REG_NRF+CONFIG, 0x00);//	掉电保护
    sta=NRF24L01_Read_Reg(STATUS);  //读取状态寄存器的值 
	  NRF24L01_Write_Reg(WRITE_REG_NRF+STATUS,(sta&0xf1));//清除TX_DS或MAX_RT中断标志
  	...
  	NRF24L01_Write_Buf(WRITE_REG_NRF+TX_ADDR,(u8*)RX_ADDRESS,RX_ADR_WIDTH);//写TX节点地址	
  	...
  	NRF24L01_Write_Reg(WRITE_REG_NRF+CONFIG, 0x0f);//配置基本工作模式的参数;PWR_UP,EN_CRC,16BIT_CRC,接收模式，关闭发送中断
...
{		
    ...
    sta=NRF24L01_Read_Reg(STATUS);  //读取状态寄存器的值 
  	...
  	NRF24L01_Write_Reg(WRITE_REG_NRF+RF_SETUP,0x0f);  //设置TX发射参数,0db增益,2Mbps,低噪声增益开启  	
  	NRF24L01_Write_Reg(WRITE_REG_NRF+CONFIG,0x0e);    //配置基本工作模式的参数;PWR_UP,EN_CRC,16BIT_CRC,接收模式，关闭发送中断
	  NRF24L01_CE_High;//CE为高,10us后启动发送
	
	  EXIT_Disable();//发送模式不要中断
